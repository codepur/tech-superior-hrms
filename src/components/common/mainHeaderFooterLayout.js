...
const MainHeaderFooterLayout = ({
  ...
  const hasSideBar =
    hasSideBarLayout && !["/set-password"].includes(router.pathname);
  ...
  return (
    <React.Fragment>
      ...
      {data?.pageScreen != "login" && data?.pageScreen != "setPassword" && (
        <HeaderComponent isPublic={isPublic} hasSideBar={hasSideBar} />
      )}
      ...
      <Row className={`${styles.mainBodyWrapper}`}>
        ...
        <Col
          md={router.pathname != "/" && `${toggle ? "10" : "11"}`}
          xs={12}
